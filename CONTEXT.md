...
| Sep 7 2026 | **Monday run failed at pre-flight: a health-check unit test still expected the OLD app-store behavior.** After the app-store fix (app-store rows now content-classified, not auto-positive), health_check §8b test "play.google.com WIZ blob → cleared → owned domain → positive" failed (got=neutral expected=positive) → `1/69 checks FAILED` → pre-flight blocked the whole run. Fix: updated that test to expect `neutral` (an app listing with no review text lands on neutral at the rule stage; production upgrades iVisa's own listing to positive via the multilingual LLM, and catches negative reviews as negative). LESSON: when classifier BEHAVIOR is intentionally changed, update the health_check §8 unit tests in the SAME commit — otherwise the next Monday pre-flight hard-fails. (Also: the app-store fix's dry-run passed but dry-run doesn't run health_check, so this only surfaced on the live Monday run.) |
| Sep 1 2026 | **News tab intermittently EMPTY (recent brand news like the HelloGov/iVisa merger vanished) — fixed.** Earned-media News count fluctuated 0–5 across weeks (Aug 31 = 0 → merger articles gone; Aug 26 = 5, incl. 4 merger mentions). Cause: `_fetch_google_news()` uses Google News (`tbm=nws`) with a custom 90-day date range (`cdr:1,cd_min,cd_max`), which Google frequently answers with NOTHING. Fix (`fetch_earned_media.py`): if the date-filtered query returns empty, retry once WITHOUT the date filter (Google News is recency-ranked by default), so recent brand news is reliably captured. Context: the HelloGov + iVisa merger (closed Jul 1 2026, announced ~Aug 19–22) — genuine third-party press = Economic Times / ET TravelWorld; most other coverage is iVisa/HelloGov OWN LinkedIn/FB/IG (correctly excluded as owned). |
| Sep 1 2026 | **Off-topic SERP results (SEMrush returned the WRONG query) — fixed.** For "que es ivisa" (ES), SEMrush returned dictionary results for the Spanish word "qué" (dle.rae.es, dequeísmo…), NONE about iVisa — but their snippets were full, so the existing "<40% snippet coverage" fallback didn't trigger and the wrong results stuck. Live Google for that query is 100% iVisa. Fix in `enrich_with_serpapi_organic()` (`fetch_serp.py`): added an OFF-TOPIC guard — if our rows mention "ivisa" in <30% of results but the live SerpAPI organic rows do in ≥50%, replace with the live organic (what users actually see). Runs alongside the snippet-coverage fallback. Verified: the "qué" rows get replaced with the real ivisa.com / trustpilot results. |
| Sep 1 2026 | **Reddit "is iVisa legit?" question mis-scored negative — prompt refined.** The ES thread "¿iVisa es legítimo? … Lo estaba pasando fatal intentando…" (a user ASKING if iVisa is legit, general travel stress, no accusation) was scored negative by the multilingual Claude classifier. Refined `_classify_with_claude()`'s prompt: negative now requires a real accusation/complaint ABOUT iVisa; someone simply ASKING whether iVisa is legit/safe (no accusation), or general travel stress/doubt, is NEUTRAL. |
